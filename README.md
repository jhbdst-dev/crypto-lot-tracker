...
프로젝트 초기에는 계산 로직을 먼저 검증하는 것이 목표였다.

실제 API를 연동하기 전에 가상 데이터를 사용하여 매수, 매도, 손익 계산이 올바르게 동작하는지 확인했다.

↓

2. Upbit API 연동

계산 로직이 검증된 후, 실제 거래 데이터를 가져오기 위해 Upbit API를 연동했다.

이를 통해 가상 데이터가 아닌 실제 거래 내역을 기반으로 프로젝트를 발전시켰다.

↓

3. PostgreSQL 저장

API를 호출할 때마다 동일한 데이터를 다시 계산하는 것은 비효율적이었다.

거래 데이터를 데이터베이스에 저장하여 이후 계산과 조회를 안정적으로 수행할 수 있도록 구조를 변경했다.
...
프론트엔드와 백엔드가 데이터를 주고받기 위해 FastAPI를 도입했다.

이전처럼 터미널에 출력하는 방식이 아니라 API를 통해 데이터를 전달하는 구조로 변경했다.

↓

7. JSON API 기반으로 변경

터미널 출력은 개발 중 검증에는 적합했지만 프론트엔드에서는 사용할 수 없었다.

출력 방식을 JSON 응답으로 변경하여 React와 연결했다.
...
이를 통해 프로젝트를 완전히 내 것으로 만들고 이후 ROS 프로젝트에도 적용할 수 있는 개발 역량을 키우고 있다.
